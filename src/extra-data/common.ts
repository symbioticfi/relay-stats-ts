...
export const computeExtraDataKeyTagged = (
...
export const collectValidatorsForSimple = (
    ...
    for (const validator of validatorSet.validators) {
        ...
        if (!payload) {
            throw new Error(
                `Failed to find key by keyTag ${tag} for validator ${validator.operator}`
            );
        }
    ...
    tuples.sort((left, right) => {
        if (left.keySerialized < right.keySerialized) return -1;
        if (left.keySerialized > right.keySerialized) return 1;
        return 0;
    });
