...
import { buildSettlementKey, cacheDelete, cacheGet, cacheSet } from './cache.js';
import { CACHE_NAMESPACE, EVENT_SCAN_RANGE, MULTICALL3_ADDRESS } from './constants.js';
import {
    CrossChainAddress,
    CacheInterface,
...
export const determineValSetStatus = async (
    ...
    let integrity: ValSetStatus['integrity'] = 'invalid';
...
const parseAnchor = (value: unknown): BlockAnchor | null => {
    ...
    if (
        typeof blockInput === 'bigint' ||
        typeof blockInput === 'number' ||
        typeof blockInput === 'string'
    ) {
    ...
    if (
        typeof bpeInput === 'bigint' ||
        typeof bpeInput === 'number' ||
        typeof bpeInput === 'string'
    ) {
        bpe = BigInt(bpeInput);
    }
...
const anchorKeyFor = (
    blockTag: BlockTagPreference | 'latest',
    settlement: CrossChainAddress
): string => `${blockTag}_${settlementKey(settlement)}`;
...
const serializeValSetLogEvent = (event: ValSetLogEvent): CachedValSetLogEvent => {
    let quorumProof: CachedValSetLogEvent['quorumProof'] = null;
    if (event.quorumProof) {
        if (event.quorumProof.mode === 'simple') {
            ...
            quorumProof = {
                ...event.quorumProof,
                signersVotingPower: event.quorumProof.signersVotingPower.toString(),
            };
        }
    }
...
const parseCachedValSetEvent = (value: unknown): ValSetLogEvent | null => {
    ...
    if (cached.kind !== ValSetEventKind.Genesis && cached.kind !== ValSetEventKind.Commit) {
        return null;
    }
    ...
    if (cached.quorumProof) {
        if (cached.quorumProof.mode === 'simple') {
            quorumProof = {
                ...
                signers:
                    cached.quorumProof.signers
                        ?.map(signer => {
                            try {
                                return new ValSetQuorumProofSimpleSigner(
                                    signer.key,
                                    BigInt(signer.votingPower)
                                );
                            } catch {
                                return null;
                            }
                        ...
                        .filter(
                            (entry): entry is ValSetQuorumProofSimpleSigner => entry !== null
                        ) ?? [],
...
const ZK_MIN_BYTES = ZK_ELEMENT_LEN * (ZK_PROOF_ELEMENTS + ZK_COMMITMENTS + ZK_COMMITMENT_POK + 1);
...
const decodeSimpleQuorumProof = (proof: Hex): ValSetQuorumProofSimple | null => {
    ...
    const aggregatedPublicKey = bytesToHex(
        bytes.slice(SIMPLE_SIG_LEN, SIMPLE_SIG_LEN + SIMPLE_PUBKEY_LEN)
    ) as Hex;

    const countStart = SIMPLE_SIG_LEN + SIMPLE_PUBKEY_LEN;
    const validatorCountBigint = bytesToBigint(
        bytes.slice(countStart, countStart + SIMPLE_COUNT_LEN)
    );
    ...
    for (let i = 0; i < validatorCount; i++) {
        ...
        const votingPower = bytesToBigint(
            bytes.slice(offset + SIMPLE_SIGNER_ENTRY_LEN / 2, offset + SIMPLE_SIGNER_ENTRY_LEN)
        );
        signers.push({
            key,
            votingPower,
...
const ingestSettlementEvents = async (
    ...
    for (const log of logs) {
        ...
        let kind = ValSetEventKind.Commit as ValSetEventKindType;
        if (log.eventName === 'SetGenesis') {
            kind = ValSetEventKind.Genesis as ValSetEventKindType;
        }
        let quorumProof: ValSetQuorumProof | null = null;
        if (metadata.transactionHash && kind === ValSetEventKind.Commit) {
            ...
            if (transactionProofCache.has(key)) {
                ...
                quorumProof = await fetchQuorumProofFromTransaction(
                    client,
...
export const retrieveValSetEvent = async (
    ...
    let toBlock = predicted + padding;
    if (toBlock > latestNumber) {
        toBlock = latestNumber;
    }
    let fromBlock = 0n;
    if (predicted > padding) {
        fromBlock = predicted - padding;
    }
    ...
    if (!event && finalized) {
        ...
        let latestFrom = 0n;
        if (latestTo > EVENT_SCAN_RANGE) {
            latestFrom = latestTo - EVENT_SCAN_RANGE;
        }
    ...
    if (event && cache) {
        await cacheSet(
            ...
            epoch,
            settlementCacheKey,
            serializeValSetLogEvent(event)
        );
        if (event.blockNumber !== null) {
            const deltaEpoch = BigInt(event.header.epoch - anchor.epoch);
            const deltaBlocks = event.blockNumber - anchor.block;
            let inferred = anchor.blocksPerEpoch;
            if (deltaEpoch > 0n && deltaBlocks > 0n) {
                inferred = deltaBlocks / deltaEpoch;
            }
            let blocksPerEpoch = anchor.blocksPerEpoch;
            if (inferred > 0n) {
                blocksPerEpoch = inferred;
            }
            ...
            await storeAnchor(
                anchorKeyFor('latest', settlement),
                updated,
                cache,
                state.anchorCache ?? new Map()
            );
        }
    }
