...
const getDriverContract = (client: PublicClient, driver: CrossChainAddress) =>
  getContract({
...
export const readDriverNumber = async <M extends DriverReadMethod>(
  ...
  const result =
    args && args.length > 0
      ? await reader(args, { blockTag })
      : await reader({ blockTag });
...
export const executeChunkedMulticall = async <T>({
  ...
  for (const chunk of chunks) {
    ...
    if (allowFailure) {
      const typed = rawResult as readonly { status: 'success' | 'failure'; result: unknown }[];
      const chunkResult = typed.map((entry) =>
        entry.status === 'success' ? (entry.result as T) : (null as T),
      );
