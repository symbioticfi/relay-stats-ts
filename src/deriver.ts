...
import { buildZkExtraData, buildSimpleExtraData } from './extra-data/index.js';
import {
  VALSET_VERSION,
  AGGREGATOR_MODE,
  AggregatorMode,
...
export class ValidatorSetDeriver {
  ...
  public async getEpochData(options?: {
    ...
    if (useCache) {
      const cached = await cacheGetTyped<ValidatorSet>(
        ...
        targetEpoch,
        cacheKey,
        (value): value is ValidatorSet => this.isValidatorSet(value) && this.canCacheValidatorSet(value),
      );
      if (cached) {
        validatorSet = cached;
      }
  ...
  private async loadAggregatorsExtraData(params: {
    ...
    if (cacheable && validatorSetHash) {
      await cacheSet(this.cache, CACHE_NAMESPACE.AGGREGATOR_EXTRA, validatorSet.epoch, cacheKey, {
